...
now = dt.now()
log_name = now.strftime("logs/T3 %m_%d_%Y  %H_%M_%S.log")
...
terminal_name = "TR3"  # which config file to load
...
def pre_entry_point():
    ...
    return render_template("TR3/unlock.html", play_boot_sound=sound_effects)
...
def foscam_control():
    ...
    version_config = get_version("TR3").get("foscam")
    config.update(version_config)
    logging.info("open CCTV page")
    return render_template("TR3/p_cctv.html", g_config=config, cams=ip_conf["cams"])
